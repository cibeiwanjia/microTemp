...
import (
	...
	"github.com/cibeiwanjia/microTemp/pkg/util"
...
func Server(cfg *conf.ConfigYaml) *http.Server {
	...
	srv := &http.Server{
		Addr:    fmt.Sprintf("%s:%d", cfg.BFF.Host, cfg.BFF.Port),
...
func setupRouter(cfg *conf.ConfigYaml) *gin.Engine {
	r := gin.New()
	middlewareOpts := []gin.HandlerFunc{
		middleware.Cors(),
		middleware.LogRequest(),
		middleware.GinLogger(logger.L),
		middleware.GinRecovery(logger.L, true),
	}
	if cfg.Chain.Enable == true {
		middlewareOpts = append(middlewareOpts, otelgin.Middleware(cfg.Chain.Name))
		middlewareOpts = append(middlewareOpts, func(c *gin.Context) {
			c.Header("Trace-Id", trace.SpanFromContext(c.Request.Context()).SpanContext().TraceID().String())
		})
	}
	r.Use(middlewareOpts...)
	// sso
